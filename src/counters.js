...
const Counter = (props) => {
  ...
  const docRef = doc(firestore, "Users", item.userID, "Projects", item.projectID, "Counters", item.key);
...
function buildName(){
  if(editMode){
    ...
    value={renameVal} 
    onChangeText={(value) => {setRenameVal(value);}} 
    style={[styles.paragraph, {backgroundColor: '#ffffff', borderRadius: 50}]}/>
...
function buildIncrementer(){
  if(editMode){
    return(<TextInput placeholder={incrementVal}
      value={newIncrement} 
      onChangeText={(value) => {
        setNewIncrement(value);}
        } 
      style={[styles.paragraph, {width: 50, backgroundColor: '#ffffff', borderRadius: 50}]}/>);
  }
  else{
    return(<Text style={[styles.paragraph, {width: 50}]}>   {incrementVal}   </Text>);
  }
}
...
function buildEditButton(){
  if(editMode){
    return(
      ...
      onPress={() => {
        ...
        if(parseInt(newIncrement)){
          console.log("newIncrement should be good? it is: ", newIncrement);
    ...
    }}><Text style={styles.paragraph}>Save</Text></TouchableOpacity>
  ...
  return(
    <View
     ...
     style={styles.container}>
      ...
      }}><Text style={styles.minusIncrementText}>-</Text></TouchableOpacity>
        {buildIncrementer()}
        <TouchableOpacity style={styles.incrementButton} 
        onPress={() => {
            incrementCounter("plus");
        }}> 
        <Text style={styles.plusIncrementText}>+</Text></TouchableOpacity>
      </View>
